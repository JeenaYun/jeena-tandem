...
parser.add_argument("--root_dir",type=str,help=": If given, root directory",default='/home/jyun/Tandem')
...
fnames = glob.glob('%s/%s/messages_*.log'%(args.root_dir,args.model_n))
for fn in fnames:
    ...
    if args.exception is not None and prefix in args.exception:
        ...
        print("tail -n 20 %s > %s/%s/summary_%s.txt"%(fn,args.root_dir,args.model_n,prefix))
        if not args.safe_mode: os.system("tail -n 20 %s > %s/%s/summary_%s.txt"%(fn,args.root_dir,args.model_n,prefix))