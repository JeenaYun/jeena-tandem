#!/usr/bin/env python3
'''
Automatically write scripts for stress perturbation (supermuc ver.)
By Jeena Yun
...
if args.write_on:
    ...
    if not args.no_pert_period:
        ...
        fshell.write('mpiexec -bind-to core -n %d tandem $setup_dir/%s --petsc -ts_checkpoint_load ../%s/step%d '
        ...
        # -- Below commented lines are for X30 model that nucleates immediately
        # fshell.write('mpiexec -bind-to core -n %d tandem $setup_dir/%s --petsc -ts_checkpoint_load ../%s/step%d '
